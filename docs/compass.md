...
| 2 | `HasDimension G n` | `n` is the **least** such dimension | `IsLeast`, not mere membership. Membership would make `DimensionFive` false: a graph with no edges fits in `ℝ⁵`, so the least edge count would be zero. |
