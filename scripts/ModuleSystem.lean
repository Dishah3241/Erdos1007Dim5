...
This executable reads each compiled Erdos1007Dim5 and project-tooling module and checks its
...
public def main : IO UInt32 := do
  -- This package and `GraphDimension` both build `scripts/*.olean`. `findOLean` stops at the
