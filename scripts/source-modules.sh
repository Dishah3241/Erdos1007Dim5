...
# Shared, fail-closed discovery of the Lean modules under Erdos1007Dim5/.
#
...
#
# The lakefile glob `Erdos1007Dim5.*` is authoritative for what is built; this function is the
...
primal_source_modules() {
  ...
  local module_path_re="^Erdos1007Dim5(/[A-Za-z_][A-Za-z0-9_']*)+\.lean$"
  local file module
  ...
  find Erdos1007Dim5 -type f -name '*.lean' -print0 | LC_ALL=C sort -z > "$files_out"
  find Erdos1007Dim5 -type l -print0 | LC_ALL=C sort -z > "$symlinks_out"
  ...
  if [ ! -s "$files_out" ]; then
    echo 'source-modules: found no Erdos1007Dim5 source files; the audit is miswired.' >&2
    return 1
  fi
