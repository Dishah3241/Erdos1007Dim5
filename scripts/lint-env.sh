...
[ "${mods:-0}" -gt 0 ] || fail "found no Erdos1007Dim5/*.lean modules — the lint is miswired"
...
run_meta do
  ...
  for idx in [0:modNames.size] do
    ...
    if let some m := modNames[idx]? then
      if m == \`Erdos1007Dim5 || (\`Erdos1007Dim5).isPrefixOf m then
...
#lint only $LINTERS in Erdos1007Dim5
...
if [ -s "$TMP/nolints-new.txt" ]; then
  echo "lint-env: @[nolint <linter>] application(s) under Erdos1007Dim5/ not accounted for in $ALLOWLIST (as '<linter> <declaration>'):"
...
awk -v pfx="$DRIVER:" '
  BEGIN { hdr = "-- Found [0-9]+ errors? in [0-9]+ declarations \\(plus [0-9]+ automatically generated ones\\) in Erdos1007Dim5 with [0-9]+ linters$" }
  {
    if (index($0, pfx) == 1) {
...
hdr_nlinters=$(sed -n 's/^.* in Erdos1007Dim5 with \([0-9]*\) linters$/\1/p' "$TMP/headers.txt")
